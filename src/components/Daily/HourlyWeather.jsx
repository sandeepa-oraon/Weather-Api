...
function HourlyWeather({forecastData, index}) {
    ...
    const handleHourlyWeather = (hourIndex) => {
        console.log('Navigating to HourlyWeatherDetail with:', index, hourIndex); // Debugging line
        navigate(`/hourlyWeatherDetail/${index}/${hourIndex}`);
    }
    return (
        <>
            <div className="Hourly-weather-">
                {forecastData &&(
                    forecastData.forecast.forecastday[index].hour.map((hourData, hourIndex) => (
                        <div key={hourIndex} className={`${hourIndex} hourly`} onClick={()=> handleHourlyWeather(hourIndex)} style={{borderBottom:'1px solid #454849', cursor:'pointer'}}>
